...
\abstract{Retrieval-Augmented Generation (RAG) improves the quality of language model responses by incorporating external knowledge. However, using a fixed number of retrieved documents (K) fails to adapt to varying query complexity, often leading to suboptimal retrieval either including irrelevant documents or missing crucial ones. To address this issue, we propose a hybrid approach that dynamically adjusts K based on query characteristics while maintaining computational efficiency. Our method improves retrieval performance by maximizing relevant information and minimizing noise. Experimental results show that it outperforms fixed-K retrieval, offering a more effective solution for optimizing RAG systems. Future work will explore learning-based techniques to further refine dynamic K selection. Future work will explore learning-based techniques to further refine dynamic K selection.}
...
Dynamic top-k Selection in Retrieval :
...
Hybrid Approaches For Top-K selection in Retrieval :
combine elements of static and dynamic K-selection to achieve a balance between simplicity and adaptability.These systems might use a static 
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|p{11cm}|}  % Use p{width} for text wrapping
		\hline
